...
public class SysShardsTest extends SQLTransportIntegrationTest {
    ...
    public void test_state_for_shards_of_closed_table() throws Exception {
        ...
        execute("select id, table_state from sys.shards where table_name = 'tbl' order by id asc");
        assertThat(response.rows()[0][0], is(0));
        assertThat(response.rows()[0][1], is("CLOSE"));
        assertThat(response.rows()[1][0], is(1));
        assertThat(response.rows()[1][1], is("CLOSE"));
    }
